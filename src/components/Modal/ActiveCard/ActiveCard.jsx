...
function ActiveCard() {
  ...
  // Function used for updating title, description, cover, comment ...
  const callUpdateCardDetailsAPI = async (updateData) => {
    const updatedData = await updateCardDetailsAPI(activeCard._id, updateData)

    // Update card being active in redux
    dispatch(updateCurrentActiveCard(updatedData))

    // update card in active board state in redux
    dispatch(updateCardInBoard(updatedData))
  }

  const onUpdateCardTitle = (newTitle) => {
    callUpdateCardDetailsAPI({ title: newTitle.trim() })
  }

  const onUpdateCardDescription = (newDescription) => {
    callUpdateCardDetailsAPI({ description: newDescription })
  }
  ...
  const onUploadCardCover = (event) => {
    ...
    toast.promise(
      callUpdateCardDetailsAPI(reqData).finally(() => event.target.value = ''),
      { pending: 'Updating...' }
    )
  }

  const onAddCardComment = async (commentToAdd) => {
    await callUpdateCardDetailsAPI({ commentToAdd })
  }

  const onUpdateCardMembers = (incomingMemberInfo) => {
    callUpdateCardDetailsAPI({ incomingMemberInfo })
  }
  return (
    <Modal
      disableScrollLock
